...
- [ ] excel文件也要考虑国际化的问题
- [X] 选择一个favicon
- [X] 奖项配置表格应该加一列用于指定当前奖项是否应用于抽奖，不应用就不在抽奖列表中展示
- [X] 抽奖列表的组件代码整理
- [X] 音效部分还有问题：开奖音乐不需要重复播放；下拉框设置音效时会重新播放音乐；试听音乐时会另起播放器，与当前音效同时播放；
- [ ] 默认数据头像的挑选及压缩
- [X] 当前正在抽取的奖项需要醒目的提示
- [X] 清空所有缓存的功能还未实现
- [X] 通用配置的功能要完善
...
- [X] 奖项拖拽调整顺序
- [ ] 各种按钮操作之后需要message反馈
- [X] 音乐整体功能，包括配置页的布局，和在抽奖页面的使用
- [X] 操作说明页的内容编写
- [ ] 最后还是要考虑一下响应式布局，应对不同尺寸的设备
- [ ] TWEEN的TS类型约束引用问题
- [ ] 清理无用代码及文件
- [X] members 修改之后还根据抽奖状态去调整状态，否则本来是卡片球，members改动之后又变成平铺了，这是BUG
- [X] 3D尺寸还是需要打磨，开奖后中奖人的卡片需要动态调整大小，要么 position.z 要么 scale，不然人数多了之后中奖卡片显得好小
- [X] 右上角的静音功能还未实现
- [X] 奖项全部抽取完毕时的交互卡住了，要修复
- [X] 批量删除在未选择项目时可以disabled
- [X] 官网展示的demo页面需要初始化填充数据，包括：人员、奖项、标题
- [X] 抽奖流程的交互还需要打磨，尤其是边界情况
- [X] 换掉 threejs 内置 TWEEN，改为独立引用
- [X] 速度曲线函数该怎么选
- [X] 图片问题整体考虑，怎么存、怎么解析，尤其是url跨域怎么办
- [X] 开奖之后显示【重抽】与【继续】按钮，在还没选择的时候切换奖项，这时就有问题了，看看交互怎么优化一下
- [X] 根据卡片的数量调整球体的大小
- [X] 根据卡片的数量调整初始平铺的缩放比例
- [X] 卡片添加头像调整布局
- [X] award添加属性repeat用于判断是否允许重复抽奖
- [X] 同时需要pool变量代替members用于存储当前可用于抽奖的人员
- [X] 卡片有头像时如何布局
- [X] BUG排查，为什么重新抽奖的时候不显示中奖人员、获奖记录也为空
- [X] 整体的配色问题：背景、卡片、中奖的卡片、开奖有没有烟花特效；
- [X] 整体的布局问题：奖项列表、标题、抽奖操作按钮、顶栏操作项
- [X] 抽奖的状态交互可能还是有BUG，逻辑还有一小部分没捋顺
- [X] 已开奖项需要增加重新抽取的入口，奖项配置列表里、首页奖项列表里都要有
...
- [X] 提供人员导入 Excel 模版下载
- [X] 人员导入的操作是否要单独开一个 Worker 线程防止阻塞
- [X] 音乐配置列表添加一列播放试听功能
