...
import { Button, Form, Popconfirm, Table, Upload } from 'antd';
...
function MusicConfig({ style }: MusicConfigProps) {
  ...
  const { musics: dataSource, columns, rowSelection, uploadProps, handleBulkDelete, handleClear, messageHolder } = useMusic(form);
  ...
  return (
    <div style={style}>
      <div className={styles.operations}>
        {messageHolder}
        <Upload {...uploadProps}>
          <Button icon={<UploadOutlined />} color="green" variant="solid" className={styles['operation-btn']}>{t('operation.add')}</Button>
        </Upload>
        <Popconfirm
          title="确认批量删除？"
